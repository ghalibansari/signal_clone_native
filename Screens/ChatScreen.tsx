...
const ChatScreen: FC<any> = ({navigation, route, route: {params: {id, chatName}}}) => {
    ...
    useLayoutEffect(() => {
        navigation.setOptions({
            ...
            headerTitleAlight: 'left',
            headerTitle: () => (
                <View style={{flexDirection: 'row', alignItems: 'center'}}>
                    <Avatar rounded  source={{uri: messages[0]?.data.photoURL || temp}} />
    ...
    useLayoutEffect(() => {
        ...
        .onSnapshot(snapshot => setMessages(
            snapshot.docs.map(doc => ({id: doc.id, data: doc.data() as IMessage['data']}))
        ))
        return unsubscribe
    }, [route])
